...
import { Search, Plus, FileText, Users, Eye, ChevronDown } from 'lucide-react';
...
interface InitiatorHistoryProps {
  userRole: string | null;
  onStartNewUpload: () => void;
  onViewRequest: (request: BulkRequest) => void;
  onLogout: () => void;
    ...
    <div className="h-full flex flex-col">
      ...
      <div className="px-[16px] py-[16px] border-b border-[#d0d5dd]">
        ...
        <div className="flex items-center gap-2 mt-1">
          ...
          <Badge className="bg-[#003883] text-white text-[11px] h-5">Initiator</Badge>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-auto p-[16px]">
        <div className="space-y-2">
          <div className="px-4 py-2 bg-[#003883] text-white rounded-lg">
            <div className="text-[14px]">Bulk Uploads</div>
          </div>
        </div>
      </div>
...
function MainContent({
  ...
  allRequests,
  currentUser
}: InitiatorHistoryProps) {
  const [activeTab, setActiveTab] = useState<'today' | 'historical'>('today');
  const [searchQuery, setSearchQuery] = useState('');
  const [statusFilter, setStatusFilter] = useState('All Statuses');
  const [showStatusDropdown, setShowStatusDropdown] = useState(false);

  const statusOptions = ['All Statuses', 'Pending Authorization', 'Approved', 'Rejected', 'Processing', 'Completed', 'Failed'];

  const breadcrumbs: BreadcrumbItem[] = [
    { label: 'CDMS Bulk Portal', screen: 'initiator-history', icon: null, current: true, isClickable: false }
  ];

  // Filter requests for current Initiator user
  const myRequests = allRequests.filter(req => req.submittedBy === currentUser);

  console.log('\n👤 INITIATOR HISTORY FILTERING:');
  console.log('Total requests in system:', allRequests.length);
  console.log('My requests (submittedBy matches):', myRequests.length);
  ...
  const todaysRequests = myRequests.filter(req => req.dateSubmitted === today);
  const historicalRequests = myRequests.filter(req => req.dateSubmitted !== today);
  ...
  const filterRequests = (requests: BulkRequest[]) => {
    ...
    if (searchQuery) {
      ...
      filtered = filtered.filter(req =>
        ...
        req.actionType.toLowerCase().includes(query)
      );
    }
  ...
  const displayedRequests = filterRequests(activeTab === 'today' ? todaysRequests : historicalRequests);
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="max-w-[1400px] mx-auto px-6 lg:px-8 py-6 lg:py-8">
          <div className="mb-8">
            <h1 className="text-[30px] text-[#101828] mb-2">Bulk Restriction Requests</h1>
            <p className="text-[16px] text-[#475467]">
              View and manage your bulk PND and Hold requests
            </p>
          </div>
        ...
        <div className="mb-6">
          <div className="bg-white border border-[#d0d5dd] rounded-[8px] shadow-sm overflow-hidden">
            <div className="flex">
              ...
              >
                <div className={`text-[14px] text-center lg:text-left ${activeTab === 'today' ? 'text-[#003883] font-semibold' : 'text-[#344054]'}`}>
              ...
              >
                <div className={`text-[14px] text-center lg:text-left ${activeTab === 'historical' ? 'text-[#003883] font-semibold' : 'text-[#344054]'}`}>
        ...
        <div className="bg-gray-50 rounded-[12px] p-4 lg:p-[20px] mb-6">
          <div className="flex flex-col lg:flex-row gap-4 lg:gap-[16px]">
            <div className="flex-1">
              ...
              <div className="bg-white rounded-[8px] border border-[#d0d5dd] shadow-sm">
                <div className="flex items-center px-[14px] py-[10px]">
                  ...
                  <Input 
                    placeholder="Search by Request ID, File Name, or Action Type"
                    value={searchQuery}
            ...
            <div className="flex-1 lg:flex-initial lg:min-w-[200px] relative">
              <label className="text-[14px] text-[#344054] block mb-2">Status</label>
              <div className="bg-white rounded-[8px] border border-[#d0d5dd] shadow-sm">
                <button
                  onClick={() => setShowStatusDropdown(!showStatusDropdown)}
                  className="w-full flex items-center justify-between px-[14px] py-[10px]"
                >
                  <span className="text-[16px] text-[#667085]">{statusFilter}</span>
          ...
          {displayedRequests.length > 0 ? (
                    ...
                    <tr key={request.id} className={`border-t border-[#eaecf0] ${index % 2 === 0 ? 'bg-white' : 'bg-[#fafafa]'}`}>
                      ...
                      <td className="py-[16px] px-[16px]">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[14px] text-[#101828]">{request.fileName}</span>
                        </div>
                      </td>
                      <td className="py-[16px] px-[16px]">
            ...
            <div className="py-16 text-center">
              ...
              <p className="text-[14px] text-[#667085]">
                {activeTab === 'today' ? 'You haven\'t submitted any requests today.' : 'No historical requests found.'}
              </p>
            </div>
          )}
        </div>
...
export function InitiatorHistory(props: InitiatorHistoryProps) {
  return (
